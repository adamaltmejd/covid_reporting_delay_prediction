...
result_par <- foreach(i = index_files)  %dopar% {
  ...
  if(write.data){
    ...
    deaths_est_T <- apply(report_lag, 1, max, na.rm=T)
    data_T <- newDeaths(deaths_est_T,
                        report_lag)
    N <- dim(data_T$death.remain)[2]
    X_T <- setup_data_lag(N, 2, result$dates_report[1:N], 2)

    Reported <- result$detected[1:pred_time,1:pred_time]
    Reported_fill <- cbind(Reported, matrix(NA, nrow=pred_time,ncol = (N_T-pred_time)))

    p <- length(res_save_lag$theta)/2
    sim <- 200#dim(res_save_lag$Death_est)[1]
    pred_set <-array(NA, dim = c(pred_time,N_T-pred_time,sim))

    Alpha_T <- matrix(NA, N_T,N_T)
    Beta_T  <- matrix(NA, N_T,N_T)
    Alpha_T[upper.tri(Alpha_T,diag=T)] <- exp(X_T%*%res_save_lag$theta[1:p])
    Beta_T[upper.tri(Beta_T,diag=T)]   <- exp(X_T%*%res_save_lag$theta[p+(1:p)])
    Alpha_T <- Alpha_T[1:pred_time,1:N_T]
    Beta_T  <- Beta_T[1:pred_time, 1:N_T]
    for(k in 1:sim){
      Reported_sample <-fill.ReportBB.lag(res_save_lag$Death_est[k,],
                                      Alpha_T,
                                      Beta_T,
                                      Reported_fill,
                                      result$dates_report[1:N_T],
                                      lag = lag)
