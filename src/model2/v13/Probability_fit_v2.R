...
library(gamm4)
library(ggplot2)
source(file.path("src","model2","v13","icu_regression.R"))
source(file.path("src","model2","v13","eval.coverage_prediction.R"))
...
probability_analysis <- function(model_dt,
                                 model_death_dt ,
                                 days.reported=5,
                                 lag = 20,
                                 prior_=NULL){
...
run.model2.all <- function(model_death_dt, model_icu_dt ,prior=T, days.to.pred=15){
   ...
   if( file.exists(file.path("data", "model", "model_seperate_lag.rds"))){
    ...
    for(i in 0:days.to.pred){
        res <- probability_analysis(model_dt,
                                    model_death_dt,
                                    days.reported=i,
                                    prior_ = prior_)
...
model_icu_dt   <- readRDS(file.path("data", "processed", "processed_data_icu.rds"))
days.reported <- 14


model_dt <- run.model2.all(model_death_dt,model_icu_dt, days.to.pred = 14)

days_age=13
coverage <- coverage_data(model_death_dt,model_dt,days.ago=days_age )
plot_coverage <- coverage.plot(coverage = coverage, days_age, "seperate lag")
print(plot_coverage)
