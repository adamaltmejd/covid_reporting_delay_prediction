...
graphics.off()
source(file.path("src","model2","v7","prob_dens.R"))
source(file.path("src","model2","v7","regression.R"))
source(file.path("src","model2","v7","model.R"))
...
d_matrix <-  as.matrix(dist(as.Date(result$Npost$dates)-as.Date(result$Npost$dates)[1]))
Sigma_GP <- matern.covariance(d_matrix, 0.2 , 1.5, mean(result$posteriror_sample$sigma_theta^2))
x11()
plot(as.Date(result$Npost$dates),Sigma_GP[1,]/Sigma_GP[1,1])
