...
model <- function(new_cases, model_paramters, prior_list, startvalue_list=NULL){
  ...
  d_matrix <- as.matrix(dist(1:(n_theta-1)))


  MH_obj_GP$Lik <- function(x, N, phi, A, Sigma, mu_mu, mu_sigma) {
    n <- length(x)
    prior_list <- prior_GP2(x[2:n], Sigma, 0)
    prior_mu_lik <- - 0.5* (x[1]-mu_mu)^2/mu_sigma^2
    prior_mu_grad <- - (x[1]-mu_mu)/mu_sigma^2
    lik_list   <- lik_negbin_theta_A(N, x, phi, A)
    res <- list(loglik = lik_list$loglik + prior_list$loglik + prior_mu_lik,
                grad = as.vector(lik_list$grad  ))
    res$grad[1]    <- res$grad[1] +  prior_mu_grad
    res$grad[2:n]  <- res$grad[2:n]+ prior_list$grad
    return(res)
  }

  # start values
  Start_theta <- log(N+1)
  MH_obj_GP$theta <- c(mean(Start_theta), Start_theta- mean(Start_theta))
  ...
  ##
  MH_obj_GP2 <- MH_setup()
  MH_obj_GP2$sigma <- 0.05
  MH_obj_GP2$Lik <- function(x, N, phi, A, Sigma, mu_mu, mu_sigma) {
      n <- length(x)
      prior_list <- prior_GP2(x[2:n], Sigma, 0)
      prior_mu_lik <- - 0.5* (x[1]-mu_mu)^2/mu_sigma^2
      prior_mu_grad <- - (x[1]-mu_mu)/mu_sigma^2
  ...
  MH_obj_GP2$theta <- c(mean(Start_theta), rep(0,length(Start_theta)))
  ...
  MH_obj_kappa_sigma$Lik <- function(x, Y, mu, distance_matrix, mu_x, Sigma_x, kappa) {
      x <- as.vector(x)
      sigma <- exp(x[1])
      Sigma_GP <- matern.covariance(distance_matrix, kappa, 1.5, sigma)

      loglik   <- lik.spatial(Y-mu, Sigma_GP)
      return(list(loglik = loglik + dmvnorm(x, mean = mu_x, sigma= Sigma_x,log=T)))
  }
  kappa <- 0.2
  ...
  theta_vec        <- matrix(0, nrow=sim,   ncol=n_theta-1)
  ...
  for(iter in 1:sim){
    ...
    MH_obj_kappa_sigma <- MHiter(MH_obj_kappa_sigma,
                                   TRUE,
                                   Y = MH_obj_GP$theta[2:n_theta],
                                   mu = 0,
    ...
    #MH_obj_kappa_sigma$theta[1] <- log(0.2)
    kappa <- 0.3
    sigma2_theta <-  sqrt(0.2)#exp(MH_obj_kappa_sigma$theta)[1]
    Sigma_GP <- matern.covariance(d_matrix, kappa, 1.5, sigma2_theta)
    L <- t(chol(Sigma_GP))
    A_Sigma <- A
    A_Sigma[,2:n_theta] <- L
    S_1 <- solve(Sigma_GP,rep(1,n_theta-1))
    S_inv <- t(rep(1,n_theta-1))%*%S_1
    #MH_obj_GP$theta[1] <- (t(S_1)%*%(A%*%MH_obj_GP$theta))/S_inv + rnorm(1)/sqrt(S_inv)

    MH_obj_GP2$theta[2:n_theta] <- solve(L,MH_obj_GP$theta[2:n_theta])
    MH_obj_GP2$theta[1]         <- MH_obj_GP$theta[1]
    MH_obj_GP2 <- MALAiter(MH_obj_GP2, TRUE,
                N = N,
                phi = phi,
                A = A_Sigma,
                Sigma = diag(dim(Sigma_GP)[1]),
                mu_mu    = mu_mu_GP,
                mu_sigma = sigma_mu_GP)
    MH_obj_GP$theta[2:n_theta] <- L%*%MH_obj_GP2$theta[2:n_theta]
    MH_obj_GP$theta[1]         <- MH_obj_GP2$theta[1]
    ...
    N_vec[iter,] <- N
    theta_vec[iter, ]   <- as.vector(A%*%as.vector(MH_obj_GP$theta))
    phi_vec[iter, ]     <- phi
    sigma_theta_vec[iter,]   <- sqrt(sigma2_theta)
    kappa_vec[iter, ]         <- kappa
    mu_GP_vec[iter, ]    <-MH_obj_GP$theta[1]
