...
\author{
	...
	Joacim Rocklöv\thanks{Department of Public Health and Clinical Medicine, Section of Sustainable Health, Umeå University}\thanks{Heidelberg Institute of Global Health, University of Heidelberg} \and
...
Moreover, deaths are reported from two distinct populations that seem to follow different trends. At the time of writing, the daily deaths in elderly care, reported with a longer delay, seem to be decreasing slower than hospital deaths. But statistics offer only aggregate numbers, prohibiting us from modeling two distinct processes. However, wew have noted a clear decline in proportions of deaths reported the two first working days. In early april .\% of deaths where reported within the first two working days wheras in late may only \% where reported during the same time interval. We adress this by assuming that the two deaths from the two first working days comes from a different population then the remainder of days.
